...
export const StartScreen: React.FC<StartScreenProps> = ({
  ...
  const [totalPlayersTop3, setTotalPlayersTop3] = useState<number>(0);
  ...
  return (
    ...
    >
      ...
      >
        A fan-made Chaos Zero Nightmare card guessing game. All asset used are
        belong to Smilegate and Super Creative.
      </motion.p>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              ...
              <div className="flex items-center mb-1">
                <motion.div
                  animate={{
                    color: ["#f59e0b", "#fbbf24", "#fcd34d", "#f59e0b"],
                  }}
                  transition={{ repeat: Infinity, duration: 3, ease: "linear" }}
                >
              ...
              <span className="text-3xl font-black text-amber-400 drop-shadow-md mb-3">
              ...
              <div className="grid grid-cols-2 gap-2 sm:gap-3 w-full max-w-sm text-[10px] sm:text-[13px]">
                  ...
                  <span className="font-bold text-amber-300">
                    {(
                      (bestScore.correctGuesses /
                        Math.max(bestScore.totalGuesses, 1)) *
                      100
                    ).toFixed(0)}
                    %
                  </span>
                ...
                <div className="col-span-2 flex items-center justify-between bg-amber-950/40 px-2 sm:px-3 py-1.5 rounded-lg border border-amber-500/10">
                  <span className="text-amber-500/70 text-[9px] sm:text-xs uppercase tracking-wider">
                    Card Pools
                  </span>
                  <span className="font-bold text-amber-300 truncate ml-2 text-[9px] sm:text-xs">
                    {bestScore.pools.character &&
                    bestScore.pools.neutral &&
                    bestScore.pools.monster &&
                    bestScore.pools.other
                      ? "ALL"
                      : [
                          bestScore.pools.character && "CHAR",
                          bestScore.pools.neutral && "NEU",
                          bestScore.pools.monster && "MON",
                          bestScore.pools.other && "OTH",
                        ]
                          .filter(Boolean)
                          .join(", ")}
                  </span>
          ...
          <div className="flex items-center justify-center mb-3 border-b border-white/5 pb-2 relative">
            <div className="flex items-center font-bold uppercase tracking-widest text-xs">
              ...
              <div className="flex items-center flex-wrap gap-2">
                <motion.span
                  className="font-bold text-transparent bg-clip-text bg-[linear-gradient(90deg,#818cf8,#c084fc,#22d3ee,#818cf8)] bg-[length:200%_auto]"
                  animate={{ backgroundPosition: ["0% center", "200% center"] }}
                  transition={{ repeat: Infinity, duration: 3, ease: "linear" }}
                >
                  Global Leaderboard
            ...
            >
              <RefreshCw
                className={`w-4 h-4 ${isLoadingTop3 ? "animate-spin text-indigo-400" : ""}`}
              />
            </button>
          </div>
          ...
          <div className="space-y-2 mb-3">
            {isLoadingTop3 ? (
              ...
              top3.map((entry, index) => (
                <div
                  key={index}
                  className="flex items-center justify-between bg-slate-900/40 rounded-xl p-2.5 border border-slate-700/50"
                >
                  <div className="flex items-center gap-2 overflow-hidden">
                    <div
                      className={`flex items-center justify-center w-8 h-8 rounded-full shrink-0 bg-slate-900 border border-slate-700/80 font-bold`}
                    >
                      <span
                        className={`text-sm ${index === 0 ? "text-amber-400" : index === 1 ? "text-slate-300" : "text-amber-600"}`}
                      >
                        {index + 1}
                      </span>
                    </div>
                    <div className="flex flex-col min-w-0">
                      <div className="flex items-center gap-1.5">
                        {index === 0 ? (
                          <Crown className="w-4 h-4 text-amber-400 drop-shadow-[0_0_3px_rgba(251,191,36,0.6)] shrink-0" />
                        ) : index === 1 ? (
                          <Medal className="w-4 h-4 text-slate-300 drop-shadow-[0_0_3px_rgba(203,213,225,0.6)] shrink-0" />
                        ) : index === 2 ? (
                        ...
                        {entry.id === playerId && (
                          <span className="px-1 py-0.5 rounded text-[8px] bg-indigo-500/20 text-indigo-300 uppercase font-black border border-indigo-500/30 shrink-0">
                            You
                          </span>
                        )}
                      </div>
                      <div className="flex bg-slate-900/50 p-1.5 rounded w-max mt-1 gap-2 items-center flex-wrap text-[9px] text-slate-500 font-medium">
                        ...
                        {entry.maxStreak !== undefined && (
                          <span className="flex items-center">
                            <Flame className="w-2.5 h-2.5 mr-0.5 text-rose-500/70" />
                            {entry.maxStreak}
                          </span>
                        )}
        ...
        <div className="flex flex-wrap gap-3 w-full justify-center">
          ...
          >
            {pools.character ? (
              ...
              <Square className="w-4 h-4 sm:w-5 sm:h-5 text-slate-500" />
            )}
            ...
            >
              ...
              <div className="overflow-y-auto space-y-3 pr-2 custom-scrollbar">
                {history.map((entry, index) => (
                  <div
                    key={index}
                    className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-4 flex flex-col gap-2 relative overflow-hidden group"
                  >
                    ...
                    <div className="flex justify-between items-center bg-slate-900/50 p-2 rounded-xl border border-slate-700/50 relative z-10">
                      ...
                      <span className="text-amber-400 text-lg font-black drop-shadow-sm">
                        {entry.score.toLocaleString()}
                      </span>
                    </div>
                    <div className="grid grid-cols-2 gap-2 text-[11px] relative z-10 w-full">
                      <div className="text-slate-400 flex justify-between bg-slate-950/40 p-1.5 rounded-lg border border-white/5">
                        <span>Timer:</span>{" "}
            ...
            >
              <div className="flex items-center justify-between mb-4 shrink-0">
                <div className="flex items-center text-indigo-400 text-lg sm:text-xl font-bold">
                  <Eye className="w-5 h-5 sm:w-6 sm:h-6 mr-2" /> Selected Card
                  Pool{" "}
                  <span className="opacity-70 ml-2 text-sm sm:text-base">
                    ({cards.length} Cards)
                  </span>
                </div>
                <button
          ...
          >
            ...
            >
              ...
              <div className="absolute top-4 right-4 z-20">
                <button
                  onClick={() => setZoomedCard(null)}
                  className="p-2 bg-black/50 hover:bg-black/80 rounded-full text-white/70 hover:text-white transition-colors backdrop-blur-md"
                >
                  <Eye className="w-5 h-5 hidden" />{" "}
                  {/* Hidden icon just for alignment if needed, or close button */}
                  <svg
